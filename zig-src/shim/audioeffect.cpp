...
void dB2string(float value, char *text, VstInt32 maxLen) {
  ...
  if (value <= 0.0000001f) {
    vst_strncpy(text, "-inf dB", maxLen);
  } else {
    float dB = 20.0f * log10f(value);
    snprintf(text, maxLen, "%.1fdB", dB);
  }
  text[maxLen - 1] = '\0';
}
...
VstInt32 dummyAudioMasterCallback(AEffect *effect, VstInt32 opcode,
                                  VstInt32 index, VstInt32 value, float opt,
                                  void *ptr) {
...
AudioEffect::AudioEffect(audioMasterCallback audioMaster)
    : fAudioMaster(audioMaster ? audioMaster : dummyAudioMasterCallback),
      fEffect(nullptr), fSampleRate(44100.0f), fBlockSize(512), fNumInputs(0),
      fNumOutputs(0), fUniqueID(0), fCanProcessReplacing(false),
      fCanDoubleReplacing(false), fProgramsAreChunks(false) {
  fEffect = new AEffect();
  memset(fEffect, 0, sizeof(AEffect));
  fEffect->magic = 0x56737450;
  fEffect->object = this;
}
...
void AudioEffect::process(float **inputs, float **outputs,
                          VstInt32 sampleFrames) {}
void AudioEffect::processReplacing(float **inputs, float **outputs,
                                   VstInt32 sampleFrames) {}
void AudioEffect::processDoubleReplacing(double **inputs, double **outputs,
                                         VstInt32 sampleFrames) {}
...
bool AudioEffect::getVendorString(char *text) {
  if (text)
    strcpy(text, "Dummy Vendor");
  return true;
}
bool AudioEffect::getProductString(char *text) {
  if (text)
    strcpy(text, "Dummy Product");
  return true;
}
bool AudioEffect::getEffectName(char *name) {
  if (name)
    strcpy(name, "Dummy Effect");
  return true;
}
...
void AudioEffect::getProgramName(char *name) {
  if (name)
    strcpy(name, "Dummy Program");
}
void AudioEffect::setProgramName(char *name) {}
void AudioEffect::getParameterLabel(VstInt32 index, char *label) {
  if (label)
    strcpy(label, "Unit");
}
...
void AudioEffect::getParameterName(VstInt32 index, char *name) {
  if (name)
    strcpy(name, "Param");
}
...
VstInt32 AudioEffect::getProgramNameIndexed(VstInt32 category, VstInt32 index,
                                            char *text) {
  return 0;
}
...
VstInt32 AudioEffect::getMidiProgramName(VstInt32 channel, VstInt32 program,
                                         char *text) {
  return 0;
}
bool AudioEffect::hasMidiPrograms() { return false; }
VstInt32 AudioEffect::getMidiProgramCategory(VstInt32 channel,
                                             VstInt32 program) {
  return 0;
}
bool AudioEffect::hasMidiProgramCategory() { return false; }
bool AudioEffect::getMidiProgramWith(VstInt32 channel, VstInt32 program,
                                     char *text) {
  return false;
}
bool AudioEffect::getMidiProgramAtr(VstInt32 channel, VstInt32 program,
                                    char *text) {
  return false;
}
bool AudioEffect::getMidiProgramNameIndexed(VstInt32 channel, VstInt32 program,
                                            char *text, VstInt32 index,
                                            bool is) {
  return false;
}
...
VstInt32 AudioEffect::getSpeakerArrangement(VstSpeakerArrangement **inputs,
                                            VstSpeakerArrangement **outputs) {
  return 0;
}
VstInt32 AudioEffect::setSpeakerArrangement(VstSpeakerArrangement *inputs,
                                            VstSpeakerArrangement *outputs) {
  return 0;
}
VstInt32 AudioEffect::getTailSize() { return 0; }
bool AudioEffect::getEffectInfo(char *name, char *vendor, char *product,
